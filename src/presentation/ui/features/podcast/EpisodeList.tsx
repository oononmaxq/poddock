  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <a href={`${basePath}/podcasts/${podcastId}/episodes/new`} className="btn btn-primary btn-sm">
          {t('episode.list.add')}
        </a>
      </div>
...
function EpisodeCard({ podcastId, episode, basePath, onStatusChange }: EpisodeCardProps) {
  const { t, lang } = useI18n();
  const { token } = useAuth();
  ...
  const handleToggleStatus = async (e: Event) => {
    ...
    try {
      const response = await fetch(`/api/podcasts/${podcastId}/episodes/${episode.id}`, {
        method: 'PATCH',
        headers: {
    ...
    <div className="card bg-base-200 hover:bg-base-300 transition-colors">
      <div className="card-body py-4 flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          >
            <span className={`text-xs ${episode.status === 'draft' ? 'text-base-content/70' : 'text-base-content/40'}`}>
            ...
            <input
              ...
              disabled={updating}
            />
            <span className={`text-xs ${episode.status === 'published' ? 'text-primary' : 'text-base-content/40'}`}>
          ...
          {episode.audio && (
            <>
              ...
              <AudioPlayer url={episode.audio.public_url} durationSeconds={episode.duration_seconds} />
            </>
          )}
        </div>
      </div>
...
function AudioPlayer({ url, durationSeconds }: { url: string; durationSeconds: number | null }) {
  ...
  const togglePlay = (e: Event) => {
    ...
    if (playing) {
      ...
      window.dispatchEvent(new CustomEvent('poddock:stopOtherPlayers', { detail: url }));
...
function EmptyState({ podcastId, basePath }: { podcastId: string; basePath: string }) {
  const { t } = useI18n();
  ...
  return (
    <div className="card bg-base-200">
      <div className="card-body items-center text-center py-8">
        ...
        <a href={`${basePath}/podcasts/${podcastId}/episodes/new`} className="btn btn-primary btn-sm mt-4">
