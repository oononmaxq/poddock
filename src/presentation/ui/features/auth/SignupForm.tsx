  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setError(data.message || t('signup.error.failed'));
      }
    } catch {
    ...
    <div className="min-h-screen bg-base-200 flex items-center justify-center">
      <div className="card w-full max-w-md bg-base-100 shadow-xl">
        <div className="card-body">
          ...
          <p className="text-center text-base-content/70 mb-6">{t('signup.subtitle')}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="form-control">
              <label className="label" htmlFor="password">
                <span className="label-text">{t('signup.password')}</span>
              </label>
              <input
                type="password"
                id="password"
            ...
            <div className="form-control">
              <label className="label" htmlFor="passwordConfirm">
                <span className="label-text">{t('signup.passwordConfirm')}</span>
              </label>
              <input
                type="password"
                id="passwordConfirm"
            ...
            <button type="submit" className="btn btn-primary w-full" disabled={loading}>
              {loading ? <span className="loading loading-spinner loading-sm" /> : t('signup.submit')}
            </button>
          </form>
          ...
          <p className="text-center text-sm text-base-content/70">
            {t('signup.hasAccount')}{' '}
            <a href="/login" className="link link-primary">{t('signup.login')}</a>
          </p>
        </div>
      </div>
    </div>
